...
export const sendPreicoTeAyudaEmailNotification = async (usuario) => {
    try {
        const nombreUsuario = usuario.name?.toUpperCase() + " " + usuario.surnames?.toUpperCase();
      ...
      const raw = JSON.stringify({
        to: "informatica@preicojuridicos.com",
        subject: "[Preico Auto App] NOTIFICACION - Preico te ayuda",
        bodyHtml: `<p>El usuario <b>${nombreUsuario}</b> con dni <b>${dniUsuario}</b> pide ayuda para solicitar los movimientos de la cuenta bancaria.</p>`,
      });
